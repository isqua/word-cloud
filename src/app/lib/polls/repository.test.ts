...
describe("PollsRepository", () => {
    ...
    describe("add", () => {
        it("should add a poll and return its id", () => {
            ...
            const poll: IPoll = {
                ...
                results: new Map(
                    pollCreateDto.initial.map((word) => [word, 0]),
                ),
            };
        ...
        it("should throw ConflictException when adding a poll with existing id", () => {
            ...
            expect(() => repository.add(pollId, poll)).toThrow(
                ConflictException,
            );
            expect(() => repository.add(pollId, poll)).toThrow(
                "Cannot use the same poll id",
            );
        });
    });
    ...
    describe("incrementWordCounts", () => {
        it("should increment word counts for existing poll", () => {
            const pollId = repository.add("test-id", {
                title: "Test Poll",
                results: new Map([["word1", 1]]),
            });
        ...
        it("should throw NotFoundException for non-existent poll", () => {
            expect(() =>
                repository.incrementWordCounts(
                    "non-existent",
                    new Set(["word"]),
                ),
